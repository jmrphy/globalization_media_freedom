...
\begin{figure}[t]
    ...
    \label{absolute}
    \begin{center}
<<Intro, cache=FALSE, fig.height=7, fig.width=6>>=
...
require(stargazer)
mainmodels<-stargazer(model1, model2, model3, model4, z.out.controls, z.out.controls.fpi,
...
As logit estimates are not conveniently interpretable, the top half of Figure 2 illustrates the long-run expected probability of observing media freedom in $country_{it}$ given different levels of international trade.\footnote{All discussion of predicted probabilities are based on 1000 simulations of the model. The simulations were conducted in R with the package \emph{Zelig} \parencite*{ZeligEveryonesSt:2009ts}.} The expected probability of observing media freedom in a country completely closed to international trade, with mean values on all other variables in the model, is \Sexpr{mean(unlist(s.out.trade$qi[1]))}. Increasing international trade from zero to the mean of \Sexpr{mean(df$lopenk2, na.rm=TRUE)} would be expected to decrease the probability of media freedom by \Sexpr{mean(s.out.trade$qi$fd)} to \Sexpr{mean(unlist(s.out.trade$qi[2]))}.

The bottom half of Figure 2 illustrates how the expected probability of observing media freedom in $country_{it}$ is expected to change in the short-run in response to yearly changes in level of international trade. The expected probability of observing media freedom in a country with a mean trade level of \Sexpr{mean(df$lopenk2, na.rm=TRUE)} but zero change in trade is \Sexpr{mean(unlist(s.out.trade.change$qi[1]))}. A yearly increase in logged trade of \Sexpr{mean(zvars.trade$dopenk2)+sd(zvars.trade$dopenk2)}, a yearly change of trade one standard deviation above the mean yearly change, would be expected to decrease the probability of media freedom by \Sexpr{mean(s.out.trade.change$qi$fd)} to \Sexpr{mean(unlist(s.out.trade.change$qi[2]))}.
...
% }
% 
% The top half of Figure 3 plots the expected probability of observing media freedom in $country_{it}$ given increases in FPI stocks. The expected probability of observing media freedom given zero FPI stock and all other variables held constant at their mean is \Sexpr{mean(unlist(s.out.fpi$qi[1]))}. An increase in FPI stock from zero to the mean value of (\Sexpr{mean(df$lfpistock2)}), would be expected to increase the probability of media freedom by \Sexpr{mean(s.out.fpi$qi$fd)} to \Sexpr{mean(unlist(s.out.fpi$qi[2]))}.
% 
% The bottom half of Figure 3 plots the expected probability of observing media freedom in $country_{it}$ in the short-run due to yearly changes in FPI stock (FPI inflows). The expected probability of observing media freedom given zero FPI inflow and all other variables held constant at their mean is \Sexpr{mean(unlist(s.out.fpi.change$qi[1]))}. The mean FPI inflow per year, (\Sexpr{mean(df$dfpistock2)}), increases the probability of media freedom by \Sexpr{mean(s.out.fpi.change$qi$fd)} to \Sexpr{mean(unlist(s.out.fpi.change$qi[2]))}.
...
\begin{figure}[t]
    ...
    \label{absolute}
    \begin{center}
<<BMA_Plot, fig.align='center', fig.height=5, cache=FALSE>>=
source("/Users/justin/Dropbox/gh_projects/globalization_media_freedom/analyses/bma.R")
@
  \end{center}
    \begin{singlespace}
        {\scriptsize{}}
...
\begin{figure}[t]
    ...
    \label{absolute}
    \begin{center}
<<BMA_Plot_Inclusions, fig.align='center', fig.height=5, cache=FALSE>>=
bma.plot.fpi
@
  \end{center}
    \begin{singlespace}
...
\subsection{Qualitative Within-Case Analysis: Argentina}
...
<<summary, results='asis', cache=FALSE, echo=FALSE, comment=FALSE, message=FALSE>>=
stargazer(full.raw, font.size="small", title="Summary Statistics")
@

\afterpage{
<<countries, results='asis', cache=FALSE, echo=FALSE, comment=FALSE, message=FALSE>>=
require(stringr)
full.raw.complete$country<-str_split_fixed(full.raw.complete$country, ",", n=2)[, 1]
stargazer(matrix(toupper(sort(unique(full.raw.complete$country))), ncol=3, byrow=TRUE), font.size="scriptsize", title="List of countries included in the statistical analyses")
@
\clearpage
}

\afterpage{
<<outlier_models, results='asis', cache=FALSE>>=
require(stargazer)
source("/Users/justin/Dropbox/gh_projects/globalization_media_freedom/analyses/outliers.R")
...
\afterpage{
<<reverse_cause_models, results='asis', cache=FALSE>>=
require(stargazer)
reverse.cause.models<-stargazer(z.out.reverse1, z.out.reverse2, z.out.reverse3,
          title="Standardized OLS Regressions: Trade Openness",
          dep.var.labels.include=FALSE,
          digits = 2,
          style = "apsr",
          font.size = "footnotesize",
          keep=c(1,2,3,4,5,6),
          notes=c("Country and year dummies are included in each model but not displayed."),
          covariate.labels=c("Media Freedom$_{t-1}$", "Democracy$_{t-1}$", "$\\Delta$Democracy", "GDP per capita$_{t-1}$", "$\\Delta$GDP per capita", "Trade$_{t-1}$")
          )
@
\clearpage
}
...
\begin{figure}[t]
    ...
    \label{absolute}
    \begin{center}
<<BMA_Plot_Supp, fig.align='center', fig.height=5, cache=FALSE>>=
source("/Users/justin/Dropbox/gh_projects/globalization_media_freedom/analyses/bma_supp.R")
@
  \end{center}
    \begin{singlespace}
        {\scriptsize{}}
...
\begin{figure}[t]
    ...
    \label{absolute}
    \begin{center}
<<BMA_Plot_Supp_Inclusions, fig.align='center', fig.height=5, cache=FALSE>>=
bma.plot.no.fpi
@
  \end{center}
    \begin{singlespace}
