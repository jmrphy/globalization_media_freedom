...
\begin{figure}[t]
    ...
    \label{absolute}
    \begin{center}
<<Intro, child='~/Dropbox/gh_projects/globalization_media_freedom/analyses/intro_graphs.Rnw', cache=FALSE>>=
...
The baseline Model 1 already correctly predicts the state of media freedom in \Sexpr{model1correct} country-years. Model 3 correctly explains \Sexpr{model3correct-model1correct} additional cases, correctly classifying \Sexpr{model3correct}.
